                    ...
                    <div className="w-[48%] flex flex-col justify-start items-start space-y-6 mt-3">
                        {isLoading ? 'Chargement en cours' : actualites.slice(0,1).map(actualite => (
                            <div className="w-full p-12 bg-[#242424] flex flex-col justify-start items-start space-y-2 relative">
                                ...
                                <p className="text-base text-[#BBBBBB]">{new Date(actualite.createdAt).toLocaleString('fr-FR', { year: 'numeric', month: '2-digit', day: '2-digit' })}</p>
                                <h4 className="text-3xl font-semibold uppercase">{actualite.title}</h4>
                                <p className="line-clamp-2 text-xl" dangerouslySetInnerHTML={{ __html: actualite.description }} />
                            </div>
                        ))}
                        {isLoading ? 'Chargement en cours' : actualites.slice(2,3).map(actualite => (
                            <div className="w-full p-12 bg-[#242424] flex flex-col justify-start items-start space-y-2 relative">
                                ...
                                <p className="text-base text-[#BBBBBB]">{new Date(actualite.createdAt).toLocaleString('fr-FR', { year: 'numeric', month: '2-digit', day: '2-digit' })}</p>
                                <h4 className="text-3xl font-semibold uppercase">{actualite.title}</h4>
                                <p className="line-clamp-4 text-xl" dangerouslySetInnerHTML={{ __html: actualite.description }} />
